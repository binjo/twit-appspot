...
(defun twit-set-auth (user pass)
  ...
  (let ((old-http-storage
         (assoc "bintweet.appspot.com:80" (symbol-value url-basic-auth-storage)))
        (old-https-storage
         (assoc "bintweet.appspot.com:443" (symbol-value url-basic-auth-storage)))
        (auth-pair
         (cons "Twitter API"
    ...
    (set url-basic-auth-storage
         (cons (list "bintweet.appspot.com:443" auth-pair)
               (cons (list "bintweet.appspot.com:80" auth-pair)
...
(defconst twit-base-url (concat twit-protocol "://bintweet.appspot.com"))
(defconst twit-secure-base-url (concat twit-protocol "://bintweet.appspot.com"))
...
(defconst twit-friend-timeline-file
  (concat twit-base-url "/statuses/friends_timeline.xml?count=40&page=%s"))
...
;;* const
(defconst twit-standard-rate-limit 150
...
;;* const
(defconst twit-time-string "%Y-%m-%d %T%z"
...
See the very end of this file for an example."
  `(let ((,url-basic-auth-storage
          (list (list "bintweet.appspot.com:80"
                      (cons "Twitter API"
                            (base64-encode-string
                             (format "%s:%s" ,user ,pass))))
                (list "bintweet.appspot.com:443"
                      (cons "Twitter API"
                            (base64-encode-string
                             (format "%s:%s" ,user ,pass))))))
         (twit-user ,user)
...
(defun twit-parse-xml-curl (url method)
  "Use CURL to retrieve file at URL with METHOD and parse with `xml-parse-fragment'."
  (let ((result nil)
        (curl-args `("-s" "-k"
                     "-X" ,method
                     "-i" ,url
...
(defun twit-get-rate-limit ()
  ...
  (let ((limit-xml (twit-parse-xml twit-rate-limit-file "GET")))
    (condition-case nil                 ; fuck xml-data with caddr/cadr...
        (twit-parse-rate-limit (cadr limit-xml))
      (error (twit-parse-rate-limit (caddr limit-xml))))))
...
XML-DATA is the sxml (with http header)."
  (if (twit-header-error-p (twit-parse-header (car xml-data)))
    ...
    (let* ((first-tweet (condition-case nil ;fuck xml-data with caddr/cadr...
                            (xml-first-child (caddr xml-data) 'status)
                          (error (xml-first-child (cadr xml-data) 'status))))
           ...
           (times-through 1))
      (dolist (status-node (condition-case nil ;fuck xml-data with caddr/cadr...
                               (xml-get-children (caddr xml-data) 'status)
                             (error (xml-get-children (cadr xml-data) 'status))))
          ...
          (save-window-excursion
            (save-excursion
              (loop for screen_name in
                    (loop for user in
                          (xml-get-children
                           ;;(caddr (twit-parse-xml twit-friend-list-url "GET"))
                           (cadr (twit-parse-xml-curl twit-friend-list-url "GET")) ; FIXME
                           'user)
                          collect (eighth user))
                           ...
                           (loop for name in
                                 (loop for user in
                                       (xml-get-children
                                        ...
                                        ;; FIXME cadr/caddr?
                                        (cadr (twit-parse-xml-curl (format twit-followers-list-url page) "GET")) 'user)
                                       collect (eighth user))
                                 collect (third name))
                           for i upfrom 1
...
Patch version from Ben Atkin."
  ...
  (pop-to-buffer
   (with-twit-buffer "*Twit-recent*"
                     ...
                     (twit-write-recent-tweets
                      (twit-parse-xml-curl (format twit-friend-timeline-file page) "GET")))))
  ...
  (pop-to-buffer
   (with-twit-buffer (concat "*Twit-at-" twit-user "*")
                     (twit-write-title "Twit @%s (Page %s) %s\n"
                                       twit-user page (format-time-string "%c"))
                     (let ((times-through 0)
                           (xml-data (twit-parse-xml-curl
                                      (format twit-mentions-url page) "GET")))
                       (dolist
                           (status-node (condition-case nil
                                            (xml-get-children (cadr xml-data) 'status)
                                          (error (xml-get-children (caddr xml-data) 'status))))
                         (twit-write-tweet status-node t times-through)
                         (setq times-through (+ 1 times-through)))))))
